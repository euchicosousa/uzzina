  ...
  return (
    <html className={cn(theme)} lang="pt-br" suppressHydrationWarning>
      <head>
        ...
        <Links />
        <script id="env-vars" type="application/json">
          {safeJsonStringify(env)}
        </script>
        <script>
          {
            "window.__env = JSON.parse(document.getElementById('env-vars').textContent || '{}');"
          }
        </script>
        <script id="theme-palette" type="application/json">
          {safeJsonStringify(PALLETE)}
        </script>
        <script>
          {`
            try {
